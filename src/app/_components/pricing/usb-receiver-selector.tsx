...
const UsbReceiverSelector: React.FC<UsbReceiverSelectorProps> = React.memo(({
  ...
  const receiverButtons = useMemo(() => 
    usbReceivers.map((receiver) => {
      const isAvailable = receiver.available !== false;
      const isSelected = selectedUsbReceiver.id === receiver.id;
      
      let buttonClasses = "flex-1 min-w-0 px-4 py-3 rounded-lg border-2 text-sm transition-all duration-200 ";
      
      if (!isAvailable) {
        buttonClasses += "border-gray-200 bg-gray-100 text-gray-400 cursor-not-allowed opacity-60";
      } else if (isSelected) {
        buttonClasses += "border-black bg-blue-900 text-white";
      } else {
        buttonClasses += "border-gray-300 hover:border-blue-300";
      }
      ...
      return (
        <button
          ...
          onClick={() => isAvailable && handleReceiverSelect(receiver)}
        ...
        >
          <div className="text-center">
            ...
            {!isAvailable && (
              <div className="text-xs font-bold mt-1 text-red-500">
                Sin stock
              </div>
            )}
            {isAvailable && receiver.additionalCostUsd > 0 && (
